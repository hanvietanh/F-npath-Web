...
import { Users, Briefcase, DollarSign, Calendar, AlertCircle, Sparkles, PieChart as PieChartIcon, Building2, UserCircle } from 'lucide-react';
import { companyProfileData, shareholderData } from '../stockDetailConstants';
...
export const ShareholdersTab: React.FC<ShareholdersTabProps> = ({ symbol, onAskCopilot }) => {
  return (
    <div className="h-full w-full bg-[#0b0e11] flex flex-col p-4 overflow-y-auto custom-scrollbar gap-4">
        ...
        {/* 4. COMPANY PROFILE & MANAGEMENT (Moved to Bottom) */}
        <div className="grid grid-cols-12 gap-4">
            {/* Company Info - Left */}
            <div className="col-span-12 xl:col-span-8 bg-[#151a21] border border-gray-800 rounded-lg p-4 flex flex-col">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-gray-800">
                    <Building2 size={16} className="text-blue-400"/>
                    <h3 className="text-sm font-bold text-white uppercase">Giới thiệu công ty</h3>
                </div>
                <p className="text-xs text-gray-300 leading-relaxed mb-6 text-justify">
                    {companyProfileData.introduction}
                    <span className="text-blue-400 cursor-pointer hover:underline ml-1 font-medium">Xem thêm</span>
                </p>
                
                <div className="grid grid-cols-2 gap-8 mt-auto">
                    <div>
                        <h4 className="text-[11px] font-bold text-gray-400 uppercase mb-2">Thông tin cơ bản</h4>
                        <div className="space-y-1.5">
                            {companyProfileData.basicInfo.map((item, i) => (
                                <div key={i} className="flex justify-between text-xs border-b border-gray-800/50 pb-1 last:border-0 hover:bg-[#1e2530] px-1 rounded transition-colors">
                    ...
                    <div>
                        <h4 className="text-[11px] font-bold text-gray-400 uppercase mb-2">Thông tin niêm yết</h4>
                        <div className="space-y-1.5">
                            {companyProfileData.listingInfo.map((item, i) => (
                                <div key={i} className="flex justify-between text-xs border-b border-gray-800/50 pb-1 last:border-0 hover:bg-[#1e2530] px-1 rounded transition-colors">
                                    <span className="text-gray-500">{item.label}</span>
                                    <span className="text-white font-medium">{item.value}</span>
                                </div>
            ...
            {/* Leadership - Right */}
            <div className="col-span-12 xl:col-span-4 bg-[#151a21] border border-gray-800 rounded-lg p-4 flex flex-col max-h-[350px]">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-gray-800 shrink-0">
                    <UserCircle size={16} className="text-emerald-400"/>
                    <h3 className="text-sm font-bold text-white uppercase">Ban lãnh đạo</h3>
                </div>
                <div className="overflow-y-auto custom-scrollbar pr-1 flex-1">
                    <table className="w-full text-xs text-left border-collapse">
                        <thead>
                            <tr className="text-gray-500 border-b border-gray-800 sticky top-0 bg-[#151a21] z-10 shadow-sm">
                                <th className="pb-2 font-medium">Họ tên</th>
                                <th className="pb-2 text-right font-medium">Chức vụ</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-800/50">
                            {companyProfileData.leaders.map((leader, idx) => (
                                <tr key={idx} className="group hover:bg-[#1e2530] transition">
                                    <td className="py-2.5 text-white font-bold">{leader.name}</td>
                                    <td className="py-2.5 text-right text-gray-400 group-hover:text-gray-300">{leader.position}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
